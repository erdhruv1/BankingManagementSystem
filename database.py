...
import datetime
from customer import Customer,Account,Savings,Current,Fixed_Deposit,Address
...
class Database:
    ...
    def sign_up_customer(self, customer):
        ...
        self.cursor.execute(sql, {"id": id, "fname": fname, "lname": lname, "password": password, "status": status, "att": att})
        ...
        self.cursor.execute(sql, {"id": id, "line1": line1, "line2": line2, "city": city, "state": state, "pincode": pincode})
        self.connection.commit()
        print("Congratulations ! Your Account was Created Successfully")
        print("Your Customer ID : ", id)

    def login_customer(self, id, password):
        sql = "select count(*) from customers where customer_id = :id and password = :password"
        self.cursor.execute(sql, {"id": id, "password": password})
        res = self.cursor.fetchall()
        count = res[0][0]
        if count == 1:
            return True
    ...
    def open_new_account_customer(self, account, cus_id):
        ...
        self.cursor.execute(sql, {"cus_id": cus_id, "acc_no": acc_no, "opened_on": opened_on, "acc_type": account_type,
                          "status": status, "bal": bal, "wd": withdrawals_left, "next_date": next_date})
        if account_type == "fd":
            term = account.get_deposit_term()
            sql = "insert into fd values (:acc_no,:amount,:term)"
            self.cursor.execute(sql, {"acc_no": acc_no, "term": term, "amount": bal})

        self.connection.commit()
        print("Account Opened Successfully")
        print("Account No is : ", acc_no)

    def change_address_customer(self, ch, id, addr):
        if ch == 1:
            sql = "update address set line1 = :line1 where customer_id = :id"
            self.cursor.execute(sql, {"line1": addr, "id": id})

        elif ch == 2:
            sql = "update address set line2 = :line2 where customer_id = :id"
            self.cursor.execute(sql, {"line2": addr, "id": id})

        elif ch == 3:
            sql = "update address set state = :state where customer_id = :id"
            self.cursor.execute(sql, {"state": addr, "id": id})

        elif ch == 4:
            sql = "update address set city = :city where customer_id = :id"
            self.cursor.execute(sql, {"city": addr, "id": id})

        elif ch == 5:
            sql = "update address set pincode = :pincode where customer_id = :id"
            self.cursor.execute(sql, {"pincode": addr, "id": id})
    ...
    def get_all_info_customer(self, id):
    ...
    def money_deposit_customer(self, account, amount):
        bal = account.get_balance()
        acc_no = account.get_account_no()
        type = "credit"
        sql = "update accounts set balance = :bal where account_no = :acc_no"
        self.cursor.execute(sql, {"bal": bal, "acc_no": acc_no})
        ...
        self.cursor.execute(sql, {"t_id": t_id, "acc_no": acc_no, "type": type, "amount": amount, "bal": bal, "date_on": date})
        self.connection.commit()

    def money_withdraw_customer(self, account, amount, msg):
        ...
        if acc_type == "savings" and msg != "transfer":
            ...
            self.cursor.execute(sql, {"wd_left": wd_left, "acc_no": acc_no})
        self.connection.commit()

    def get_transactions_account(self, acc_no, date_from, date_to):
        ...
        self.cursor.execute(sql, {"acc_no": acc_no, "date_from": date_from, "date_to": date_to})
        res = self.cursor.fetchall()
        return res

    def transfer_money_customer(self, account_sender, account_receiver, amount):
        if account_sender.withdraw(amount) == True:
            account_receiver.deposit(amount)
            self.money_withdraw_customer(account_sender, amount, "transfer")
            self.money_deposit_customer(account_receiver, amount)
            print("Transfer Completed !")
            print("New Balance for Account No ", account_sender.get_account_no(), " : ", account_sender.get_balance())
            print("New Balance for Account No ", account_receiver.get_account_no(), " : ",
                  account_receiver.get_balance())
    ...
    def close_account_customer(self, account):
        acc_no = account.get_account_no()
        ...
        self.cursor.execute(sql, {"acc_no": acc_no, "closed_on": closed_on})
        print("Account Closed Successfully !")
        print("Rs ", balance, " will be delivered to your address shortly")
        self.connection.commit()

    def get_loan_customer(self, acc_no, loan_amt, loan_term):
        sql = "select loan_id_sequence.nextval from dual"
        self.cursor.execute(sql)
        res = self.cursor.fetchall()
        loan_id = res[0][0]
        sql = "insert into loans values (:acc_no,:loan_id,:amount,:loan_term)"
        self.cursor.execute(sql, {"acc_no": acc_no, "loan_id": loan_id, "loan_term": loan_term, "amount": loan_amt})
        self.connection.commit()
    ...
    def reset_login_attempts(self, id):
    ...
    def update_customer(self, customer):
        ...
        self.cursor.execute(sql, {"status": status, "att": att, "id": id})
        self.connection.commit()
